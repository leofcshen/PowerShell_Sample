...
try {
	...
	$TargetFolerName = ((Get-Item $PSCommandPath).BaseName -split "_")[1]
	$TargetFolerPath = "$PSScriptRoot\$TargetFolerName"
	$Today = Get-Date -Format "yyyyMMdd"
	$IsRunBackup = $true
	$BackupFolderNewName = "${TargetFolerName}_${Today}"
	$FolderList = (Get-ChildItem -Path $BackupBase -Directory -Filter "${BackupFolderNewName}*").Name
